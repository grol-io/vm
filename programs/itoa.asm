...
itoa_digits_loop:
    ...
    IncrS -1 idx ; decrement idx by 1 (which thus also increments the length=maxbuf-idx)
    LoadS num
    JNE 0 itoa_digits_loop
itoa_done:
    LoadS sign ; sign
    JEQ 1 itoa_finish_str ; positive, so done/no need to add the minus sign
    LoadI '-'
    StoreSB buf idx ; stores '-' in buf at offset indicated by idx
    IncrS -1 idx ; idx by -1
itoa_finish_str:
    LoadI maxbuf ; calculate length based on what we started idx at
    SubS idx
