// Code generated by "stringer -type=Instruction"; DO NOT EDIT.

package cpu

import "strconv"

func _() {
	// An "invalid array index" compiler error signifies that the constant values have changed.
	// Re-run the stringer command to generate them again.
	var x [1]struct{}
	_ = x[invalidInstruction-0]
	_ = x[LoadI-1]
	_ = x[AddI-2]
	_ = x[JNZ-3]
	_ = x[LoadR-4]
	_ = x[AddR-5]
	_ = x[StoreR-6]
	_ = x[Sys-7]
	_ = x[lastInstruction-8]
}

const _Instruction_name = "invalidInstructionLoadIAddIJNZLoadRAddRStoreRSyslastInstruction"

var _Instruction_index = [...]uint8{0, 18, 23, 27, 30, 35, 39, 45, 48, 63}

func (i Instruction) String() string {
	idx := int(i) - 0
	if i < 0 || idx >= len(_Instruction_index)-1 {
		return "Instruction(" + strconv.FormatInt(int64(i), 10) + ")"
	}
	return _Instruction_name[_Instruction_index[idx]:_Instruction_index[idx+1]]
}
